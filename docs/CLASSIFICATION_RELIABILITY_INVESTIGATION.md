...
3. **Evidence-coverage fixes** (small, parallel): add Israel Hayom `/sport/israeli-soccer/` + audit all category maps (walla is entirely unmapped — check whether its URLs carry categories); add transfer-market football vocabulary ("דמי מעבר", "עסקת העברה") to football context; add "על סף סיכום" to negotiation; taxonomy: Budućnost, the handful of relevant players, and register "הצהובים" as a **shared** alias of both Maccabi TLV entities so the existing ambiguity machinery handles it (`signal-taxonomy-change`). **[Superseded — #64 R6 Q3, 2026-07-11]:** the "הצהובים" registration was rejected on corpus evidence. It is ambiguous *within basketball* (Maccabi Ramat Gan / Kiryat Gat are also "yellow" clubs, so a unique Maccabi-TLV resolution would let one club capture another's articles), redundant with the co-present full name in nearly all hits, and bare "צהוב" collides with "כרטיס צהוב" (yellow card). The colour-nickname family ("האדומים"/"הירוקים") is worse (every Hapoel + Man United + red-card false positives). Decision: **do not broaden nicknames**; abstention is correct and is now test-locked (`TestNicknameAbstentionQ3`). See `docs/TAXONOMY.md` → "Nickname breadth audit (#64 R6 Q3)". The C13 Maccabi-negotiation miss it targeted is a *sport-resolution* gap (the alias already exists; the sport is `unknown`), tracked separately.
