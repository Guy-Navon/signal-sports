...
- **User Platform enforcement is live (PRs 1–3, 5, 6 / #49–#51, #53, #54).** Email/password accounts, cookie sessions, the session-derived `/api/me/*` consumer surface, the frontend auth shell, and fail-closed admin gating of the legacy `{user_id}`/ops surface are all main-branch behavior; the test suite runs under real enforcement with explicit identity fixtures (anonymous/user/admin). Onboarding UX (#52, PR #74) is review-gated; account lifecycle (#55) remains.
