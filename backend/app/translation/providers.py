...
from app.translation.translation_quality import looks_like_hebrew_translation
...
class ClaudeTranslationProvider(TranslationProvider):
    ...
    def translate_title_to_hebrew(self, text: str, source_language: str) -> Optional[str]:
        try:
            ...
            if not looks_like_hebrew_translation(result, text):
                logger.warning(
                    "Translation quality check failed for %r — got %r", text, result
                )
                return None
            return result
