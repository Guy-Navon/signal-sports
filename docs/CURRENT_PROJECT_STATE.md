...
Last updated: 2026-07-14 (Milestone 5 — Real Story Clustering v1 COMPLETE and **dark-shipped**; `CLUSTERING_ENABLED=false` is the production default).

**2026-07-14 — REAL STORY CLUSTERING v1 COMPLETE (Milestone 5, #99–#105) — DARK-SHIPPED, OFF BY DEFAULT.** The full deterministic capability is built, merged and acceptance-tested end to end: contract + frozen fixtures (#99), matcher (#100), persistence + live ingestion stage (#101), backfill/repair/QA tooling (#102), feed/API cluster collapse (#103), frontend card + Debug evidence (#104), dual-mode acceptance (#105). **`CLUSTERING_ENABLED=false` is the production default and remains so.**
